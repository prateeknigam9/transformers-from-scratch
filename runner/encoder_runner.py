...
# DATA HANDLING
    # loading the data
    data_path = data_scripts.fetch_data("stefanlarson/outofscope-intent-classification-dataset")
    # preprocessing 
    ...
    model = model_scripts.encoder_model.build_encoder_only_transformer(
                                            ...
                                            seq_len=max_seq_len+1).to(device)
    print("\n The Model Summary: \n")
    ...
    training_scripts.train_encoder.train(model, train_loader, val_loader,
                                         config["n_epochs"], config["device"],
                                         optimizer, loss_fn)
    
    utils.metrics.predict_on_sample(model,tokenizer,max_seq_len,
                                    train_ds, config, id_to_class, train_data,n=5)
